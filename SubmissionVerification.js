...
window.location.href = "/sites/Finance/ExpenseClaimSystem/SitePages/SubmissionVerificationMobile.aspx";
});
...
function loadTable(){
var requestUri = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OtherExpenseClaims')/items?$filter=(substringof('"+loggedInUserEmail +"',PendingWithEmail) and Status eq 'Pending' and PendingWith eq 'Submission And Verification')&$top=5000";
    tableContent = "";
    tableContents = "";
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            tableContents = '<table id="example" class="display table-bordered" style="width:100%;"><thead><tr><th scope="col"></th><th scope="col" id="thClaimID">Claim ID</th><th scope="col">Account Head Details</th><th scope="col">Manager\'s Remarks</th><th scope="col">Employee Name</th><th scope="col">Claim Date</th><th scope="col">Claimed INR</th><th scope="col">View Claim</th>';
            ...
            for (var i = 0; i < objItems.length; i++) {
            	var mgrID = "mgr"+objItems[i].ID;
	            	...
	            	if(accountHeadDetails != ""){
	            		...
	            		tableContent += '<td><table class="table table-bordered" id="AccountHeadTable"><thead><tr><th>Account Head(s)</th><th>Particular(s)</th><th>Claimed Amount</th><th>Approved Amount</th></tr></thead><tbody><tr>No contents available</tr></tbody></table></td>';
	            	}
...
function approveClaim(ev, claimID, count, checkBoxLength,totClaimedAMount ){
		debugger;
		var pendingWith = "Authorization";
		var pendingWithEmail = "Authorization";
...
$.ajax({
        ...
        success: function(data) {
        	...
        	if(objItems.length == 1)
        	{
				...
				L4approverEmail = data.d.results[0].L4ApproverMail;
				status = data.d.results[0].Status;
				if(status == "Pending" && loggedInUserEmail.toLowerCase() == currentPendingWith.toLowerCase())
				{
					pendingWith = data.d.results[0].L4Approver;
					pendingWithEmail = data.d.results[0].L4ApproverMail+"-Authorization";
					count++;
