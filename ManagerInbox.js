...
window.location.href = "/sites/Finance/ExpenseClaimSystem/SitePages/ManagerInboxMobile.aspx";
});
            ...
            if(data.d.results.length != 0)
            {
            	for(var i =0;i<data.d.results.length;i++){
            		...
            		if(todayDate > date1 && todayDate< date2 )
            		{
            			...
            			URLToLoad =_spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OtherExpenseClaims')/items?$filter=((PendingWithEmail eq '"+loggedInUserEmail +"' or PendingWithEmail eq '"+delegatedFor +"') and Status eq 'Pending')&$top=5000";
            		...
            		{
            			URLToLoad =	 _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OtherExpenseClaims')/items?$filter=PendingWithEmail eq '"+loggedInUserEmail +"' and Status eq 'Pending'&$top=5000";
...
//function loadTable(URLToLoad,Delegated){
function loadTable(loggedInUserEmail){
debugger;
URLToLoad =	 _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('OtherExpenseClaims')/items?$filter=PendingWithEmail eq '"+loggedInUserEmail +"' and Status eq 'Pending'&$top=5000";
var requestUri = URLToLoad;
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            tableContents = '<table id="example" class="display table-bordered" style="width:100%;text-align: center; "><thead><tr style="color: white;background-color: #0072c6;"><th scope="col"></th><th scope="col">Claim ID</th><th scope="col">Account Head Details</th><th scope="col">Manager\'s Remarks</th><th scope="col">Employee Name</th><th scope="col">Claim Date</th><th scope="col">Claimed INR</th><th scope="col">View Claim</th>';
            ...
            for (var i = 0; i < objItems.length; i++) {
            	var mgrID = "mgr"+objItems[i].ID;
	            	...
	            	if(accountHeadDetails != ""){
	            		...
	            		tableContent += '<td><table class="table table-bordered"><thead><tr><th>Account Head(s)</th><th>Particular(s)</th><th>Claimed Amount</th><th>Approved Amount</th><th>Attachment</th></tr></thead><tbody><tr>No contents available</tr></tbody></table></td>';
	            	}
	            	
	            	//manager's remark
	            	tableContent += '<td><textarea rows="4" class="managersArea" id="'+mgrID+'" style="width:100%"></textarea></td>';
...
debugger;
                		...
                		//debugger;	
                		//alert($innerRow.find("td:nth-child(3)").text())	
...
if($('input[name="checkboxClaim"]').is(':checked')){
		$("input[name='checkboxClaim']:checked").each(function(){
			...
			var claimID = (row.cells[1].innerHTML).split("D")[1];
			var managerRemarks = row.cells[2].innerHTML;
			...
			if(countAHRows > 1 )
			{
					...
					if(typeof approvedAmount == "undefined" || approvedAmount == "")
					{
						...
						bootbox.alert("Approved amount Cannot be blank");
					}//end of undefined check
					else if(typeof approvedAmount != "undefined" && claimedAmount != approvedAmount)
					{
						if(managerRemarks  == ""){
						//remarksClaims= remarksClaims + ",CCD"+claimID;
						//msg ="\n"+remarksClaims+"Please enter remarks.";
						bootbox.alert("Kindly enter remarks")
						isRemarksFilled = false;
						}
						else
...
debugger;
				...
				//approveClaim(claimID, accountHead, particulars, claimedAmount, approvedAmount, checkBoxLength, this);
					...
					if(managerRemarks == ""){
						...
						return;
							...
							// deleteClaimDetails("CCD"+claimID);
							//saveOtherExpenseClaimDetails("CCD"+claimID, accountHead, particulars, claimedAmount,approvedAmount);
							UpdateClaimDetails(DetailsID , accountHead, particulars, claimedAmount,approvedAmount,"Pending");}
							}
					}
				}
				else
	...
	function UpdateClaimDetails(ID, accountHead, particulars, claimedAmount,approvedAmount,status){
var total = 0;
		var item = {
		    "__metadata": {
...
function approveinOtherExpense(ev, claimID, count, checkBoxLength,totClaimedAMount,pendingWith,pendingWithEmail)
{
	    ...
	    $.ajax({
			...
			success: function(data) {
						addtoHistory("CCD"+claimID,loggedInUserName,"Approved",""); 
