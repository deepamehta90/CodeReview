...
$(document).on('click','#MyOtherExpenseClaims',function(e){
...
function loadTable(){
debugger;
var requestUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('TelephoneBroadbandClaims')/items?$filter=EmpEmail eq '"+LoginUserEmail+"'&$top=4999";
	            ...
	            /*	//Approved INR
                  if(objItems[i].TotalApprovedAmount != null && objItems[i].TotalApprovedAmount != "" ){
	            	...
	            	*/
	            	//Claim ID - view
                  if(objItems[i].ID != null && objItems[i].ID != "" ){
                  	      tableContent += '<td><a href="/sites/Finance/ExpenseClaimSystem/SitePages/ViewMobileExpense.aspx?User?'+objItems[i].ID+'" target="blank"><i class="fa fa-eye" aria-hidden="true" style="font-size: 16px;"></i></a></td>';

	            		/*tableContent += '<td><a href="/sites/Finance/ExpenseClaimSystem/SitePages/ViewOtherExpense.aspx?User?'+objItems[i].ID+'" target="blank">View Claim</a></td>';*/
	            	}else{
	            		tableContent += '<td></td>';
	            	}
...
//Claim ID-delete
	...
	//tableContent += '<td><button type="button" class="btn btn-xs btn-succes" onclick=DeleteClaim('+objItems[i].ID+');>Delete </button></td>';
...
//Claim ID-print
                  if(objItems[i].ID != null && objItems[i].ID != "" ){
	            		if(objItems[i].Status == "Pending" && objItems[i].PendingWith == "Submission & Verification")
                  		{
                  		tableContent += '<td><a href="/sites/Finance/ExpenseClaimSystem/SitePages/PrintClaim.aspx?'+objItems[i].ID+'" target="blank"><i class="fa fa-print" style="font-size: 16px;"></i></a></td>';
                  		}
                  		else
...
$(document).on('click','.downloadFiles',function(){
                		...
                		debugger;				    
						ID = $row.find("td:nth-child(1)").text().split('M')[1];
						//alert(ID);
...
function attachementFetch(id) {
              var image = [];
			var call = $.ajax({
			        url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('TelephoneBroadbandClaims')/items(" + id + ")/AttachmentFiles",
			        type: "GET",
			        dataType: "json",
					async: false,
			        headers: {
...
function DeleteClaim(ID)
{
    ...
    bootbox.confirm({
    title: "",
    message: "Are you sure to delete M"+ID+"?",
    buttons: {
        cancel: {
    ...
    {
       $.ajax({  
            url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('TelephoneBroadbandClaims')/items(" + ID+ ")",  
            type: "POST",  
            ...
            success: function(data) {  
            bootbox.alert("Claim ID M"+ID+" deleted successfully.", function(){ 
    				location.reload();				});
                          },  
            error: function(data) {  
                alert("failed");  
